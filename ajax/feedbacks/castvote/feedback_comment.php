...
$user_feedback_vote = get_feedback_vote(USER['user_id'], $feedback_comment_id);

if (empty($user_feedback_vote)) {
	cast_feedback_vote(USER['user_id'], $feedback_comment_id, $vote);
} else {
	$user_feedback_vote = $user_feedback_vote[0];

	if ($user_feedback_vote['vote'] == $vote) {
		delete_feedback_vote(USER['user_id'], $feedback_comment_id);
	} else {
		cast_feedback_vote(USER['user_id'], $feedback_comment_id, $vote);
	}
}

$feedback_votes = get_feedback_votes($feedback_comment_id);
$user_feedback_vote = get_feedback_vote(USER['user_id'], $feedback_comment_id);
