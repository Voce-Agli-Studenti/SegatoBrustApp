...
function get_feedback_comments_full($feedback_id) {
  ...
  COALESCE(
    (
			SELECT
        SUM(feedback_comment_votes.vote)
      FROM
        feedback_comment_votes
      WHERE feedback_comment_votes.feedback_comment_id = feedback_comments.feedback_comment_id)
		, 0) AS vote_count
FROM
  feedback_comments
 ...
 * @param boolean $is_anonymous Indica se il commento è anonimo
 *
 * @return string|boolean In caso di successo ritorna il feedback_id, altrimenti ritorna false
 */
function add_feedback_comment($feedback_id, $user_id, $text, $is_anonymous) {
	$pdo = pdo_connection();
